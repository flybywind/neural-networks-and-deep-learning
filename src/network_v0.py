...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        nabla_b = [np.zeros(b.shape, self.mini_batch_size) for b in self.biases]
        ...
        for b, w in zip(self.biases, self.weights):
            b = np.repeat(b, self.mini_batch_size, 1)
        ...
        delta = self.cost_derivative(activations[-1], y) * \
            sigmoid_prime(zs[-1])
        nabla_b[-1] = np.dot(delta, np.ones([self.mini_batch_size, 1]))
        ...
        for l in xrange(2, self.num_layers):
            z = zs[-l]
            sp = sigmoid_prime(z)
            delta = np.dot(self.weights2[-l+1].transpose(), delta) * sp
